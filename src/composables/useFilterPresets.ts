...
export function useFilterPresets(
  ...
  const manualFilterChips = computed((): FilterChip[] => {
    ...
    if (manualFilters.value && typeof manualFilters.value === 'object') {
  ...
  function rebuildQuickFilters() {
    ...
    const activePresets = allPresets.value.filter((p) => activePresetIds.value.includes(p.id));
    ...
    if (activePresets.length === 0) {
      ...
      const validFilters = activePresets
        .map((p) => p.filter)
  ...
  async function savePreset(preset: Omit<FilterPreset, 'id'>) {
    if (!preset.filter) return; // Skip if no filter
  ...
  async function deletePreset(preset: FilterPreset) {
    ...
    updateLayoutOptions({
      ...
      activeQuickFilterId:
        activePresetIds.value[0] === preset.id
          ? undefined
  ...
  function removeManualFilter(chipId: string) {
    ...
    if (
      manualFilters.value &&
      typeof manualFilters.value === 'object' &&
      field in manualFilters.value
    ) {
      const fieldFilter = (manualFilters.value as any)[field];
      if (operator && fieldFilter && typeof fieldFilter === 'object' && operator in fieldFilter) {
        delete fieldFilter[operator];

        // If no more operators for this field, remove the field
        if (Object.keys(fieldFilter).length === 0) {
          delete (manualFilters.value as any)[field];
        }
      }
    }
  }
  ...
  function updatePreset(preset: FilterPreset, updates: Partial<FilterPreset>) {
    ...
    // Ensure filter is not null for QuickFilter
    if (updatedFilter.filter) {
      updatedFilters[index] = updatedFilter as QuickFilter;
    }
