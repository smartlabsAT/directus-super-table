<template>
	<div class="inline-edit-wrapper">
		...
		>
			...
			<div class="edit-popover">
				...
				<div class="popover-footer" v-if="!autoSave">
					<v-button x-small secondary @click="cancelEdit">
						Cancel
					</v-button>
					<v-button x-small :disabled="!hasChanges" @click="saveAndClose">
						Save
					</v-button>
				</div>
			...
			<div class="drawer-content">
				...
				<div class="file-browser-inline">
					...
					<div v-else class="files-grid">
						<!-- No content message -->
						<div v-if="availableFiles.length === 0 && availableFolders.length === 0" class="no-files">
		...
		<v-dialog
			...
			@esc="showClearConfirmDialog = false"
		>
			<v-card>
				...
				<v-card-actions>
					<v-button secondary @click="showClearConfirmDialog = false">
						Cancel
					</v-button>
...
const isLongText = computed(() => {
	return props.fieldType === 'text' || 
		   (typeof props.value === 'string' && props.value.length > 100);
});

const isJsonField = computed(() => {
	return props.fieldType === 'json' || 
